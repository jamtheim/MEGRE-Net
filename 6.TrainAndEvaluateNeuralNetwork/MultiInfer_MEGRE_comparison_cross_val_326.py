...
# Edited to support DICE evaluations for multiple inferences performed at different training iteration timepoints
# Christian 2019-07-12
...
infiter = sys.argv[4]			# Set what training iteration inference to use images from


data_dir = '/mnt/md1/Christian/Projects/MEGRE/Code/MEGRE/models/MEGRE_iter_' + str(iter_nbr) + '_GT' + Radius # The path to the results of iter run iter_nbr 
write_dir = data_dir + "/"
ground_truth_dir = '/mnt/md1/Christian/Projects/MEGRE/Patients326processedPooledAugN4v2/GT' + Radius + "/"  # The path to the ground truth


text_file_name = 'MEGRE_iter_' + str(iter_nbr) + '_GT' + Radius + '_comparison_crossval_' # The name of the text file that will contain the results
nbr_cross_vals = np.sum(['cross_' in folder_name for folder_name in os.listdir(data_dir)]) # Find number of cross val folders
""" """
# print(nbr_cross_vals)
...
for kCross in range(nbr_cross_vals):
	...
	output_dir = current_dir + "output_all_" + infiter + "/"
	...
	fileName = text_file_name + str(kCross + 1) + '_i' + infiter + ".csv"
...
fileName = "Cross_val_summary" + "_MEGRE_iter_" + str(iter_nbr) + "_GT" + Radius + "_i" + infiter + ".csv"
...
df.to_csv(write_dir + "Patients_summary" + "_MEGRE_iter_" + str(iter_nbr) + "_GT" + Radius + "_i" + infiter + ".csv", index=False)
